...
// Faculty self-registration (first time)
...
exports.login = async (req, res) => {
  try {
    ...
    if (!user.isRegistered || !user.password) return res.status(400).json({ message: 'Account not activated. Contact admin.' });
    if (user.role !== role) return res.status(403).json({ message: `This account is not a ${role}` });
    ...
    res.json({
      token,
      user: {
        _id: user._id, name: user.name, employeeId: user.employeeId,
        email: user.email, role: user.role, department: user.department,
        designation: user.designation, assignedSubjects: user.assignedSubjects
      }
    });
  } catch (err) {
    res.status(500).json({ message: err.message });
...
// Check if employee exists (for "existing faculty" check)
